...
def _screen_sys(brand):
    return (
        f"You are a meticulous QA analyst for {brand}. You detect statements on the "
...
def _verify_sys(brand):
    return (
        f"You verify flagged claims for {brand} and DISCARD false positives. Be strict: "
...
def _screen_user(brand, facts_c, features_c, body):
    return (
        f"SOURCE-OF-TRUTH FACTS:\n{facts_c}\n\nFEATURES:\n{features_c}\n\n"
        f"From the PAGE TEXT, list every claim about {brand} that (a) contradicts or "
        ...
        "a language/region/country/regulation/translation count, or (e) is any other internal "
        ...
        '"category":"database_size|positioning|free_claim|language_count|region_count|regulation_count|other_mismatch",'
    ...
    def make(page):
        async def run():
            ...
            s1 = await llm.call_json(task="fact_check", model=llm.fast_model,
                                     cache_key=f"{h}|fc_s1", system=_screen_sys(brand),
                                     user=_screen_user(brand, facts_c, features_c, body))
            ...
            s2 = await llm.call_json(task="fact_check", model=llm.reasoning_model,
                                     cache_key=f"{h}|fc_s2", system=_verify_sys(brand),
